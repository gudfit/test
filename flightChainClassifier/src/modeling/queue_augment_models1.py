...
class QTSimAM(nn.Module):
    ...
    def __init__(self, e_lambda=1e-4, lambda_queue=1.0):
        ...
        self.lambda_queue = nn.Parameter(
            torch.tensor(lambda_queue, dtype=torch.float32)
        )
    ...
    def forward(self, x, delta):
        ...
        for t in range(seq_len):
            ...
            delta_t = (
                delta[:, t]
                if t < delta.size(1)
                else torch.zeros(batch_size, device=x.device)
            )
            # Concatenate previous hidden state and delta_t.
            mod_input = torch.cat(
                [h_t, delta_t.unsqueeze(1)], dim=1
            )  # (B, hidden_size+1)
...
class QTSimAM_CNN_LSTM_Model(nn.Module):
    ...
    def __init__(
        ...
        self.delta_estimator = nn.Sequential(nn.Linear(1, 1), nn.Sigmoid())

        final_cnn_channels = self.cnn_channels_list[-1]
        # Use the QMogrifierLSTM to process the CNN features.
        self.qmogrifier_lstm = QMogrifierLSTM(
