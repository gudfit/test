...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding='same', activation=nn.ReLU, dropout_rate=0.0):
        super().__init__()
        layers = [
...
class StandardLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bidirectional, dropout_rate):
        super().__init__()
        self.lstm = nn.LSTM(
    ...
    def forward(self, x):
        ...
        # We usually need the final hidden state for classification
