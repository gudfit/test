...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = (
            x * channel_weights
        )  # Broadcast multiplication: (B, C, S) * (B, C, 1) -> (B, C, S)

        # Apply Spatial Attention
        spatial_weights = self.SpatialAtt(x_out)  # (Batch, 1, SeqLen)
        x_out = (
            x_out * spatial_weights
        )  # Broadcast multiplication: (B, C, S) * (B, 1, S) -> (B, C, S)
        return x_out
...
class SimAM(torch.nn.Module):
    ...
    ):  # channels arg is not used in original SimAM, but good practice
    ...
    def forward(self, x):
        ...
        e_inv = numerator / denominator + 0.5
