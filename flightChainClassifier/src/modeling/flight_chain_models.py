...
class CBAM_CNN_Model(nn.Module):
    ...
    def __init__(
        ...
        dropout_rate=None,
    ):  # Dropout rate for ConvBlocks
        super().__init__()

        # --- Set Parameters (Use provided or default from config) ---
        cnn_channels_param = (
            cnn_channels if cnn_channels is not None else config.DEFAULT_CNN_CHANNELS
        )
        kernel_size_param = (
            kernel_size if kernel_size is not None else config.DEFAULT_KERNEL_SIZE
        )
        dropout_rate_param = (
            dropout_rate if dropout_rate is not None else config.DEFAULT_DROPOUT_RATE
        )
        ...
        self.pool = nn.AdaptiveAvgPool1d(
            1
        )  # Global Average Pooling -> (Batch, LastChannel, 1)
...
class SimAM_CNN_LSTM_Model(nn.Module):
    ...
    def __init__(
        ...
        lstm_hidden_param = (
            lstm_hidden if lstm_hidden is not None else config.DEFAULT_LSTM_HIDDEN_SIZE
        )
        lstm_layers_param = (
            lstm_layers if lstm_layers is not None else config.DEFAULT_LSTM_NUM_LAYERS
        )
        lstm_bidir_param = (
        ...
        cnn_layers = []
        print(
            f"Building SimAM_CNN_LSTM_Model with CNN channels: {self.cnn_channels_list}, LSTM hidden: {lstm_hidden_param}, layers: {lstm_layers_param}, dropout: {dropout_rate_param}"
        )
        for i in range(len(self.cnn_channels_list) - 1):
            in_c = self.cnn_channels_list[i]
            out_c = self.cnn_channels_list[i + 1]
        ...
        print(
            f"  Added LSTM(input={final_cnn_channels}, hidden={lstm_hidden_param}, layers={lstm_layers_param}, bidir={lstm_bidir_param})"
        )

        # --- Classifier Head ---
        # Input size is determined by LSTM hidden size and directionality
        lstm_output_features = (
            lstm_hidden_param * 2 if lstm_bidir_param else lstm_hidden_param
        )
