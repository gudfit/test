...
"""
Unified training loop for all flight-chain models (CBAM, SimAM, QTSimAM).

Key features
------------
* Optional jitter / synthetic-chain augmentation (config.USE_SIM_AUG & --no-aug).
* Auto-selection of reasonable LSTM hidden size & layer count.
* Saves best model plus a tiny *.meta.json* with architecture params.
"""
...
def run_training(
    *,
    ...
    if not stats:
        print("❌  data stats missing – run chain constructor first"); sys.exit(1)
    num_feat = stats["num_features"]

    # ── 2. dataset choice  (plain vs simulated) ────────────────
    ...
    # ── 3. auto hyper-params  (unless passed explicitly) ───────
    lstm_layers      = lstm_layers      or (hyperparams or {}).get("lstm_num_layers", 1)
    lstm_hidden_size = lstm_hidden_size or (hyperparams or {}).get("lstm_hidden_size", 128)
    ...
    # ── 4. class-weights  *after* train_ds exists  ─────────────
    ...
    if model_type.lower() == "cbam":
        ...
        print(f"Unknown model type {model_type}"); sys.exit(1)

    model = model.to(device)
    optim_ = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    sched  = optim.lr_scheduler.ReduceLROnPlateau(optim_, "min", factor=0.2,
                                                  patience=3, min_lr=1e-7)

    # ── 6. training loop ───────────────────────────────────────
    best_val = float("inf")
    meta = {"lstm_hidden_size": lstm_hidden_size,
            "lstm_num_layers":  lstm_layers,
            "dropout":          dropout,
            "sim_factor":       config.SIM_FACTOR if config.USE_SIM_AUG else 1}

    print(f"🏋️  hidden={lstm_hidden_size}  layers={lstm_layers}  "
    ...
    for ep in range(1, epochs + 1):
        ...
        sched.step(va_loss)
        dur = time.time() - t0
        print(f"Epoch {ep:02d}/{epochs} | {dur:5.1f}s "
              f"| Train {tr_loss:.4f}/{tr_acc:.3f}"
              f" | Val {va_loss:.4f}/{va_acc:.3f}", flush=True)
        # save best
        if va_loss < best_val:
            best_val = va_loss
