...
# For simulation-based augmentation
...
class FlightChainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(chain, dtype=torch.float32), torch.tensor(label, dtype=torch.long)
...
class FlightChainDatasetSim(Dataset):
    ...
    def __init__(self, chains_file, labels_file, data_stats, sim_factor=None):
        ...
        print(f"Loaded simulated dataset with base size={len(self.base_chains)} and SIM_FACTOR={self.sim_factor}")
...
if __name__ == "__main__":
    ...
    required = [config.TRAIN_CHAINS_FILE, config.TRAIN_LABELS_FILE, config.DATA_STATS_FILE]
    ...
    ds_sim   = FlightChainDatasetSim(config.TRAIN_CHAINS_FILE, config.TRAIN_LABELS_FILE, stats)
