# flightChainClassifier/src/main1.py
...
try:
    from src import config  # Import config to use its helper functions (load_best_hyperparameters, etc.)
...
def main():
    ...
    # Override config SUBSAMPLE_DATA if specified via command line
    ...
    if args.use_best_params:
        if not args.skip_train:  # Only load parameters if training is not skipped
    ...
    if not args.skip_data:
        ...
        # Check if necessary processed data files exist when skipping data processing.
    ...
    if not args.skip_train:
        ...
        try:
            if args.model == 'qtsimam':
                # Import the new queue-augmented model.
        ...
        print("\n--- Skipping Training ---")
        # Check if model file exists when skipping training but not evaluation.
    ...
    if not args.skip_eval:
        ...
        try:
            if args.model == 'qtsimam':
                # Import the queue-augmented model for evaluation.
                from src.modeling.queue_augment_models import QTSimAM_CNN_LSTM_Model
    ...
    # Restore original config SUBSAMPLE_DATA if it was overridden.
