...
try:
    ...
    from src.training.trainer import validate_epoch  # reuse the common validation function
except ImportError:
    # Fallback if run directly or if paths need adjustment
    ...
    try:
        ...
        print(f"CRITICAL: Error importing modules in hyperparameter_tuning_qtsimam.py: {e}")
        sys.exit(1)
...
def objective(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        return float('inf')
    print(f"Trial {trial.number} Parameters: lr={lr:.1E}, weight_decay={weight_decay:.1E}, dropout_rate={dropout_rate:.2f}, lstm_hidden_size={lstm_hidden_size}, lstm_num_layers={lstm_num_layers}")
    ...
    epochs_for_tuning = config.EPOCHS  # you may reduce this for tuning speed, e.g. 10 epochs
    for epoch in range(epochs_for_tuning):
        ...
        avg_train_loss = total_train_loss / train_samples if train_samples > 0 else float('inf')

        # Validate using the common validation function
        val_loss, val_acc = validate_epoch(model, val_loader, criterion, device)
        print(f"Epoch {epoch+1}/{epochs_for_tuning}: Train Loss: {avg_train_loss:.4f}, Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.4f}")
    ...
    print(f"Trial {trial.number} finished with Best Val Loss: {best_trial_val_loss:.4f}")
...
if __name__ == "__main__":
    ...
    if complete_trials:
        ...
        try:
            ...
            best_params_file = config.BEST_PARAMS_FILE.with_name("best_hyperparameters_qtsimam.json")
