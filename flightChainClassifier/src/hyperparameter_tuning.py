...
# (Keep the full function definition as provided before)
def objective(trial: optuna.Trial):
    ...
    try:
        ...
        batch_size = config.BATCH_SIZE # Keep fixed for now
        optimizer_name = "Adam"
    except Exception as e: return float('inf') # Suggest failed
    ...
    # --- Load Data & Weights ---
    try:
        data_stats = config.load_data_stats(); num_features = data_stats['num_features']
        ...
        if len(unique_classes_train) > 1:
             try: class_weights = compute_class_weight('balanced', classes=np.arange(config.NUM_CLASSES), y=train_dataset.labels); class_weights_tensor = torch.tensor(class_weights, dtype=torch.float32).to(device)
             except ValueError: print("  Warn: Could not compute weights.");
    ...
    except Exception as e: print(f"Error loading data: {e}"); return float('inf')
    # --- Initialize Model ---
    try:
    ...
    except Exception as e: print(f"Error initializing model: {e}"); return float('inf')
    # --- Loss & Optimizer ---
    criterion = nn.CrossEntropyLoss(weight=class_weights_tensor)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    # --- Training Loop ---
    best_trial_val_loss = float('inf')
    epochs_for_tuning = config.EPOCHS # Or maybe fewer epochs just for tuning? e.g., 15
    for epoch in range(epochs_for_tuning):
        epoch_start_time = time.time(); model.train(); total_train_loss = 0.0; train_samples = 0
        for chains, labels in train_loader:
            chains, labels = chains.to(device), labels.to(device)
            optimizer.zero_grad(); outputs = model(chains); loss = criterion(outputs, labels)
            if torch.isnan(loss): continue
            loss.backward(); optimizer.step()
            total_train_loss += loss.item() * chains.size(0); train_samples += labels.size(0)
        avg_train_loss = total_train_loss / train_samples if train_samples > 0 else 0
        val_loss, val_acc = validate_epoch(model, val_loader, criterion, device) # Reuse validation func
        epoch_duration = time.time() - epoch_start_time
        # print(f"    Epoch {epoch+1}/{epochs_for_tuning} | Train L: {avg_train_loss:.4f} | Val L: {val_loss:.4f} | Val Acc: {val_acc:.4f} | Time: {epoch_duration:.2f}s") # Less verbose
        best_trial_val_loss = min(best_trial_val_loss, val_loss)
        trial.report(val_loss, epoch)
        if trial.should_prune(): print(f"  Trial {trial.number} pruned at epoch {epoch+1}."); return best_trial_val_loss if np.isfinite(best_trial_val_loss) else float('inf')
        if not np.isfinite(val_loss) or val_loss > 100.0: print(f"  Trial {trial.number} aborted: unstable val loss ({val_loss})."); return float('inf')
...
# (Keep the __main__ block exactly as provided before, it handles study creation, optimization, results display, saving best params, and plotting)
if __name__ == "__main__":
    print("Checking for processed data files...")
    required_files = [config.TRAIN_CHAINS_FILE, config.TRAIN_LABELS_FILE, config.VAL_CHAINS_FILE, config.VAL_LABELS_FILE, config.DATA_STATS_FILE]
    if not all(f.exists() for f in required_files):
        print("Error: Processed data files (.npy, .json) not found."); sys.exit(1)
    else: print("Processed data files found.")
    print("\n--- Starting Hyperparameter Tuning with Optuna ---")
    study_name = "simam-cnn-lstm-tuning-v1"; n_trials = 20; study_timeout_hours = 2
    study = optuna.create_study(direction="minimize", pruner=optuna.pruners.MedianPruner(n_warmup_steps=3, n_min_trials=5))
    study_start_time = time.time()
    try: study.optimize(objective, n_trials=n_trials, timeout=study_timeout_hours * 3600, n_jobs=1)
    except KeyboardInterrupt: print("\n--- Tuning stopped manually ---")
    except Exception as e: print(f"\n--- Error during tuning: {e} ---"); traceback.print_exc()
    finally: study_duration = time.time() - study_start_time; print(f"\n--- Optuna Study Finished (Duration: {study_duration:.2f}s) ---")
    pruned_trials = [t for t in study.trials if t.state == optuna.trial.TrialState.PRUNED]; complete_trials = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE]; fail_trials = [t for t in study.trials if t.state == optuna.trial.TrialState.FAIL]
    print(f"\nStudy Summary: Total={len(study.trials)}, Complete={len(complete_trials)}, Pruned={len(pruned_trials)}, Failed={len(fail_trials)}")
    if complete_trials:
        best_trial = study.best_trial; print("\nBest trial completed:"); print(f"  Number: {best_trial.number}, Value (Loss): {best_trial.value:.6f}"); print("  Params: ");
        ...
        except Exception as e: print(f"Error saving best parameters: {e}")
    else: print("\nNo trials completed successfully.")
    if complete_trials or pruned_trials:
        try:
            if optuna.visualization.is_available():
                print("\nGenerating Optuna visualization plots..."); config.PLOTS_DIR.mkdir(parents=True, exist_ok=True)
                fig1 = optuna.visualization.plot_optimization_history(study); fig1.write_image(config.PLOTS_DIR / f"{study_name}_optuna_history.png")
                if len(complete_trials) >= 1: fig2 = optuna.visualization.plot_param_importances(study); fig2.write_image(config.PLOTS_DIR / f"{study_name}_optuna_param_importance.png")
