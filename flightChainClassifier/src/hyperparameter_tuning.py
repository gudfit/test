...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Hyperparameter Tuning for SimAM CNN-LSTM Model"
    )
    parser.add_argument(
        "--balanced",
        action="store_true",
...
def objective(trial: optuna.Trial):
    ...
    try:
        ...
        return float("inf")
    print(
        f"Trial {trial.number} Parameters: LR={lr:.1E}, WD={weight_decay:.1E}, Dropout={dropout_rate:.2f}, LSTMHidden={lstm_hidden_size}, LSTMLayers={lstm_num_layers}"
    )
    # --- Load Data & Weights ---
    try:
    ...
    for epoch in range(epochs_for_tuning):
        ...
        if not np.isfinite(val_loss) or val_loss > 100.0:
            ...
            return float("inf")
    print(
        f"--- Trial {trial.number} Finished. Best Val Loss: {best_trial_val_loss:.4f} ---"
    )
...
if __name__ == "__main__":
    ...
    try:
        study.optimize(
            objective, n_trials=n_trials, timeout=study_timeout_hours * 3600, n_jobs=1
        )
        ...
        try:
            if optuna.visualization.is_available():
                ...
                if len(complete_trials) >= 1:
                    ...
                    fig2.write_image(
                        config.PLOTS_DIR / f"{study_name}_optuna_param_importance.png"
                    )
