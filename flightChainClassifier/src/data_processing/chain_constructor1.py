...
warnings.simplefilter(action='ignore', category=FutureWarning) # Ignore numpy/pandas future warnings if any
...
try:
    ...
    try:
        ...
        print("CRITICAL: Cannot find 'config.py'. Ensure it's in the current working directory or src/ directory and PYTHONPATH is set correctly.")
        sys.exit(1)
...
def parse_hhmm(time_col_series):
    ...
    time_str = time_col_series.astype(str).replace({r'\.0$': ''}, regex=True) # Remove .0 from floats
    time_str = time_str.replace({'nan': pd.NA, 'None': pd.NA}) # Handle string 'nan'/'None'
    ...
    parsed_time = pd.to_datetime(time_str.where(valid_time_mask), format='%H%M', errors='coerce').dt.time
    return parsed_time
...
def load_raw_data(file_path):
    ...
    try:
        ...
        if missing_req_cols:
             print(f"Warning: Required columns missing from raw file and will be skipped: {missing_req_cols}")
        ...
        dtype_spec = {col: str for col in ['Tail_Number', 'Reporting_Airline', 'Flight_Number_Reporting_Airline', 'Origin', 'Dest'] if col in available_cols}
        df = pd.read_csv(file_path, usecols=available_cols, dtype=dtype_spec, low_memory=False)
...
def clean_data(df):
    ...
    critical_cols_check = [
        'Tail_Number', 'FlightDate', 'CRSDepTime', 'CRSArrTime',
        config.TARGET_COL_RAW, 'Origin', 'Dest', 'DepTime', 'ArrTime',
        'CRSElapsedTime'
    ]
...
def engineer_features(df):
    ...
    try:
        features_df['FlightDate_dt'] = pd.to_datetime(features_df['FlightDate'], errors='coerce').dt.date
    ...
    except Exception as e: print(f"Error during basic time parsing: {e}"); sys.exit(1)

    features_df['SchedDepDateTime'] = features_df.apply(combine_date_time, axis=1, date_col='FlightDate_dt', time_col='CRSDepTime_t')
    features_df['SchedArrDateTime'] = features_df.apply(combine_date_time, axis=1, date_col='FlightDate_dt', time_col='CRSArrTime_t')
    features_df['ActualDepDateTime'] = features_df.apply(combine_date_time, axis=1, date_col='FlightDate_dt', time_col='DepTime_t')
    features_df['ActualArrDateTime'] = features_df.apply(combine_date_time, axis=1, date_col='FlightDate_dt', time_col='ArrTime_t')

    datetime_cols_check = ['SchedDepDateTime', 'SchedArrDateTime', 'ActualDepDateTime', 'ActualArrDateTime']
    initial_rows = len(features_df); features_df.dropna(subset=datetime_cols_check, inplace=True)
    print(f"Dropped {initial_rows - len(features_df)} rows with invalid datetime parsing/combination.")
    if features_df.empty: print("Error: No valid rows after datetime processing."); sys.exit(1)

    for sched_col, base_col in [('SchedArrDateTime', 'SchedDepDateTime'), ('ActualArrDateTime', 'ActualDepDateTime')]:
    ...
    numerical_features_base = [
        'DepDelayMinutes', 'CRSElapsedTime', 'ActualElapsedTime', 'AirTime', 'Distance',
        'TaxiOut', 'TaxiIn' ]
    numerical_features = [col for col in numerical_features_base if col in features_df.columns]
    ...
    categorical_features = ['Reporting_Airline', 'Origin', 'Dest', 'Flight_Number_Reporting_Airline']
    ...
    required_cols_later = ['Tail_Number', 'DateTime_ToSortBy', config.TARGET_COL_RAW,
                           'SchedArrDateTime', 'ActualDepDateTime', 'ActualArrDateTime'] # Ensure ActualArrDateTime is here
    cols_to_keep_initial = list(set(required_cols_later + features_to_process))

    # Check if all initially kept columns exist
    missing_keep_cols = [col for col in cols_to_keep_initial if col not in features_df.columns]
    if missing_keep_cols:
        print(f"Error: Columns needed for processing/chaining are missing *before* feature engineering: {missing_keep_cols}")
        sys.exit(1)
    ...
    for col in categorical_features: features_df[col] = features_df[col].fillna('__MISSING__').astype(str)
    encoder = OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1)
    features_df[categorical_features] = encoder.fit_transform(features_df[categorical_features])
    encoder_categories = {col: cats.tolist() for col, cats in zip(categorical_features, encoder.categories_)}
    ...
    scaler = MinMaxScaler()
    features_df[numerical_to_scale] = scaler.fit_transform(features_df[numerical_to_scale])
    scaler_params = {'min': scaler.min_.tolist(), 'scale': scaler.scale_.tolist(), 'feature_names': numerical_to_scale}
    ...
    print(f"Final features for model input ({num_final_features}): {final_feature_cols}")

    # Store stats
    data_stats = {'num_features': num_final_features, 'feature_names': final_feature_cols,
                  'encoder_categories': encoder_categories, 'scaler_params': scaler_params}
...
def create_chains(df, feature_cols, target_col, stats):
    ...
    for col in required_dt_cols:
        if col not in df.columns: print(f"Error: Required datetime column '{col}' missing."); sys.exit(1)
        try: df[col] = pd.to_datetime(df[col], errors='coerce')
        except Exception as e: print(f"Error converting column '{col}' to datetime: {e}"); sys.exit(1)
    df.dropna(subset=required_dt_cols, inplace=True)
    if df.empty: print("Error: DataFrame empty after ensuring valid datetimes."); sys.exit(1)
    ...
    max_ground_seconds = config.MAX_GROUND_TIME.total_seconds() if config.MAX_GROUND_TIME is not None else None
    ...
    for i in range(len(df) + 1): # Iterate one index beyond the end
        ...
        if should_process_group:
            ...
            if group_len >= config.CHAIN_LENGTH:
                ...
                for j in range(start_idx_group, end_idx_group - config.CHAIN_LENGTH + 1):
                    idx_f1, idx_f2, idx_f3 = j, j + 1, j + 2

                    # --- Validation Logic (using NumPy arrays) ---
                    valid_chain = True
                    # Check 1: Actual Dep f2 > Actual Arr f1 + Min Turnaround
                    if actual_dep_times_sec[idx_f2] <= (actual_arr_times_sec[idx_f1] + min_turnaround_sec):
                        valid_chain = False
                    # Check 2: Actual Dep f3 > Actual Arr f2 + Min Turnaround
                    if valid_chain and actual_dep_times_sec[idx_f3] <= (actual_arr_times_sec[idx_f2] + min_turnaround_sec):
                        valid_chain = False
                    # Check 3: Max Ground Time (Actual Arr to Actual Dep)
                    if valid_chain and max_ground_seconds is not None:
                        ground_time12_sec = actual_dep_times_sec[idx_f2] - actual_arr_times_sec[idx_f1]
                        ground_time23_sec = actual_dep_times_sec[idx_f3] - actual_arr_times_sec[idx_f2]
                        if ground_time12_sec > max_ground_seconds or ground_time23_sec > max_ground_seconds:
                            valid_chain = False
                    ...
                    try:
                        label_class = pd.cut([label_raw], bins=config.DELAY_THRESHOLDS, labels=False, right=True)[0]
                        if pd.isna(label_class):
                            if label_raw <= config.DELAY_THRESHOLDS[1]: label_class = 0
                            else: label_class = config.NUM_CLASSES - 1
                    except Exception as e:
    ...
    print(f"Tail number groups processed (>= {config.CHAIN_LENGTH} flights): {processed_groups_count}")
    print(f"Skipped {skipped_short} tail numbers with < {config.CHAIN_LENGTH} flights.")
    print(f"Skipped {skipped_validation} potential chains due to time validation failures.")
...
def run_chain_construction():
    ...
    if config.TARGET_COL_RAW not in df_featured.columns: print(f"Error: Target column '{config.TARGET_COL_RAW}' not found."); sys.exit(1)
    if 'DateTime_ToSortBy' not in df_featured.columns: print(f"Error: Sorting key 'DateTime_ToSortBy' not found."); sys.exit(1)
    if config.SUBSAMPLE_DATA < 1.0:
         print(f"!!! SUBSAMPLING featured data to {config.SUBSAMPLE_DATA*100:.1f}% BEFORE CHAIN CREATION !!!")
         df_featured = df_featured.sample(frac=config.SUBSAMPLE_DATA, random_state=config.RANDOM_STATE).copy()
         print(f"Using {len(df_featured)} rows for chain creation.")
         if df_featured.empty: print("Error: Data empty after subsampling."); sys.exit(1)
    chains_np, labels_np = create_chains(df_featured, final_cols, config.TARGET_COL_RAW, data_stats)
