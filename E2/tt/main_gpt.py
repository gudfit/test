...
from src.model_wrapper import load_model_and_tokenizer, reconstruct_sentence
from src.metrics import calculate_semantic_similarity
from src.analysis import analyze_and_plot 

SEMANTIC_THRESHOLD = 0.95

def run_gpt_experiment_v2(config: dict, sentences: list[str]) -> pd.DataFrame:
    """
    Runs the full experimental loop for Autoregressive Models (GPT), now including
    ...
    for lambda_budget in config['lambda_budgets']:
        ...
        for sentence in tqdm(sentences, desc=f"Processing sentences for {model_name}"):
            for theta_budget in config['theta_budgets']:
                ...
                is_perfect_reconstruction = (reconstructed_text.strip() == sentence.strip())
...
def main_gpt_v2():
    """ 
    Main function to run the entire enhanced GPT pipeline and analyze BOTH metrics. 
    """
    config_path = 'configs/experiment_config.yaml'
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    
    output_filename = 'gpt_semantic_results.csv'
    config['output_file'] = os.path.join('results', output_filename)
    print("--- GPT Semantic Experiment Configuration Loaded ---")
    print(yaml.dump(config, indent=2))
    print("\n--- Loading Data ---")
    sentences = get_sentences_from_dataset(config)
    print("\n--- Starting GPT Semantic Experiment ---")
    results_df = run_gpt_experiment_v2(config, sentences)
    ...
    output_dir_base = os.path.join("results", "analysis_gpt_semantic")
