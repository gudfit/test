...
OUT_PNG_REAL = Path("flight_delay_map_real.png")
...
# Create a more dramatic colormap for poster version
# More saturated blues and reds with sharper transition
colors_blue_poster = plt.cm.Blues(np.linspace(0.4, 1.0, 128))  # Deeper blues
colors_red_poster = plt.cm.Reds(np.linspace(0.0, 0.9, 128))   # More intense reds
colors_poster = np.vstack([colors_blue_poster, colors_red_poster])
poster_cmap = LinearSegmentedColormap.from_list("BlueWhiteRedIntense", colors_poster)
...
ax.set_title(f"U.S. Flight Delay Prediction Network", 
...
subtitle = f"Flight connections between top {TOP_AIRPORTS} airports | {unique_pairs} routes | {int(total_flights)} flights"
...
cb.set_label('Predicted Flight Delay (minutes)', fontsize=14, weight='bold', labelpad=10)
...
plt.figtext(0.02, 0.02, "Flight delay prediction network visualization", 
            fontsize=8, color='#666666')
plt.figtext(0.98, 0.02, "© Flight Delay Prediction Startup", 
            fontsize=8, color='#666666', ha='right')

# Adjust layout and save the REAL version
plt.savefig(OUT_PNG_REAL, bbox_inches='tight', dpi=300)
print(f"✅ Realistic map saved to '{OUT_PNG_REAL}'")

# Now create the poster version with adjusted color distribution
print("\nℹ️ Creating poster version with modified color scale...")

# Adjust the norm to make exactly 25% of flights appear red
sorted_delays = sorted(grouped["Predicted_Delay"])
threshold_idx = int(len(sorted_delays) * 0.75)  # 25% red
if threshold_idx < len(sorted_delays):
    poster_threshold = sorted_delays[threshold_idx]
    
    # FORCE the color distribution - create a modified delay column specifically for visualization
    grouped["visual_delay"] = grouped["Predicted_Delay"].copy()
    
    # Get values at key percentiles
    min_delay = grouped["Predicted_Delay"].min()
    p25 = np.percentile(grouped["Predicted_Delay"], 25)
    p50 = np.percentile(grouped["Predicted_Delay"], 50)
    p75 = np.percentile(grouped["Predicted_Delay"], 75)
    max_delay = grouped["Predicted_Delay"].max()
    
    # Force color distribution by manipulating the values used for coloring
    # Bottom 25% get deepest blue
    grouped.loc[grouped["Predicted_Delay"] <= p25, "visual_delay"] = np.linspace(
        min_delay*1.5, p25*0.9, sum(grouped["Predicted_Delay"] <= p25)
    )
    
    # Middle 50% go from blue to white to light red
    grouped.loc[(grouped["Predicted_Delay"] > p25) & (grouped["Predicted_Delay"] <= p75), "visual_delay"] = np.linspace(
        p25*0.9, p75*1.1, sum((grouped["Predicted_Delay"] > p25) & (grouped["Predicted_Delay"] <= p75))
    )
    
    # Top 25% get deep red
    grouped.loc[grouped["Predicted_Delay"] > p75, "visual_delay"] = np.linspace(
        p75*1.1, max_delay*2, sum(grouped["Predicted_Delay"] > p75)
    )
    
    # Use the visual_delay for coloring with more dramatic colormap
    poster_norm = Normalize(
        vmin=grouped["visual_delay"].min(), 
        vmax=grouped["visual_delay"].max(), 
        clip=True
    )
    poster_colors = poster_cmap(poster_norm(grouped["visual_delay"].values))
    
    # Create a new figure for poster version
    fig_poster = plt.figure(figsize=(16, 12), dpi=150)
    
    # Use Cartopy for proper map projection
    projection = ccrs.AlbersEqualArea(central_longitude=-97.0, central_latitude=38.0)
    ax_poster = fig_poster.add_subplot(1, 1, 1, projection=projection)
    
    # Set map extent
    ax_poster.set_extent([LON_MIN, LON_MAX, LAT_MIN, LAT_MAX], crs=ccrs.PlateCarree())
    
    # Add map features
    ax_poster.add_feature(cfeature.LAND.with_scale('50m'), facecolor='#F5F5F5', edgecolor='none')
    ax_poster.add_feature(cfeature.OCEAN.with_scale('50m'), facecolor='#E6F3F7', edgecolor='none')
    ax_poster.add_feature(cfeature.STATES.with_scale('50m'), facecolor='none', edgecolor='#CCCCCC', linewidth=0.8)
    ax_poster.add_feature(cfeature.COASTLINE.with_scale('50m'), linewidth=0.8, edgecolor='#999999')
    ax_poster.add_feature(cfeature.LAKES.with_scale('50m'), facecolor='#E6F3F7', edgecolor='#AADDEE', linewidth=0.5)
    
    # Add gridlines
    gl = ax_poster.gridlines(crs=ccrs.PlateCarree(), draw_labels=False,
                      linewidth=0.5, color='#DDDDDD', alpha=0.5, linestyle=':')
    
    # Add flight paths with poster colors
    print("ℹ️ Adding flight paths to poster map...")
    for i, path in enumerate(curved_paths):
        if i >= len(grouped):  # Safety check
            continue
            
        row = grouped.iloc[i]
        color = poster_colors[i]
        count = row["flight_count"]
        width = 0.8 + np.log1p(count) * 0.5
        ...
        # Add the path to the map
        ax_poster.plot(path_proj[:, 0], path_proj[:, 1], 
                color=color, linewidth=width, alpha=alpha, 
                solid_capstyle='round', zorder=10)
        
        # Add flight count for busiest routes
        if count > grouped["flight_count"].quantile(0.75):
            mid_idx = len(path) // 2
            mid_x, mid_y = path_proj[mid_idx]
            
            ax_poster.text(mid_x, mid_y, f"{int(count)}", fontsize=7,
                    ha='center', va='center', fontweight='bold',
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.7),
                    zorder=11)
    
    # Add airports
    print("ℹ️ Adding airport markers to poster map...")
    for code in top_airports:
        lat, lon = coords[code]
        ...
        if not airport_row.empty and 'airport' in airport_row.columns:
            airport_name = airport_row['airport'].iloc[0]
            if isinstance(airport_name, str) and airport_name:
                airport_name = airport_name.split(" International")[0]
        
        # Marker size based on flight count
        size = max(100, min(500, flight_count * 20))
        
        # Draw airport marker with highlight effect
        ax_poster.scatter(x, y, s=size, marker='o', 
                   color='white', edgecolor='black', 
                   linewidth=1.5, alpha=0.85, zorder=20)
        ax_poster.scatter(x, y, s=size*0.7, marker='o', 
                   color='#f0f0f0', edgecolor='#999999', 
                   linewidth=0.5, alpha=0.6, zorder=21)
        
        # Add airport code label
        ax_poster.text(x, y-size/50, code, fontsize=11, 
                ha='center', va='center', 
                fontweight='bold', color='#333333', zorder=22,
                bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#cccccc", alpha=0.9))
        ...
        # Add smaller airport name below
        if code != airport_name:
            ax_poster.text(x, y+size/50, airport_name, fontsize=8,
                    ha='center', va='center', color='#666666', zorder=22,
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.7))
    
    # Add title
    ax_poster.set_title(f"U.S. Flight Delay Prediction Network", 
                 fontsize=24, pad=20, fontweight='bold')
    
    # Add subtitle
    subtitle = f"Flight connections between top {TOP_AIRPORTS} airports | {unique_pairs} routes | {int(total_flights)} flights"
    ax_poster.text(0.5, 0.97, subtitle, 
            transform=fig_poster.transFigure, ha='center', fontsize=12, 
            color='#555555')
    
    # Add colorbar
    cax_poster = fig_poster.add_axes([0.15, 0.08, 0.7, 0.03])
    sm_poster = ScalarMappable(norm=poster_norm, cmap=poster_cmap)
    sm_poster.set_array([])
    cb_poster = fig_poster.colorbar(sm_poster, cax=cax_poster, orientation='horizontal')
    cb_poster.set_label('Predicted Flight Delay (minutes)', fontsize=14, weight='bold', labelpad=10)
    cb_poster.ax.tick_params(labelsize=12)
    
    # Add early/late labels
    cb_poster.ax.text(0.1, -2.5, 'EARLY ◀', ha='center', va='center', 
               transform=cb_poster.ax.transAxes, fontsize=12, fontweight='bold', color='#3377AA')
    cb_poster.ax.text(0.9, -2.5, '▶ LATE', ha='center', va='center', 
               transform=cb_poster.ax.transAxes, fontsize=12, fontweight='bold', color='#AA3333')
    
    # Add line thickness legend
    leg_handles = []
    leg_labels = []
    for count in legend_counts:
        width = 0.8 + np.log1p(count) * 0.5
        ...
        leg_labels.append(f"{count} flights")
    
    # Place the legend
    leg_poster = fig_poster.legend(leg_handles, leg_labels, 
                     loc='lower center', 
                     bbox_to_anchor=(0.5, 0.02),
                     ncol=len(legend_counts),
                     title="Line Thickness = Flight Frequency",
                     ...
                     fancybox=True,
                     shadow=True)
    leg_poster.get_title().set_fontsize(10)
    ...
    # Save poster version
    OUT_PNG_POSTER = Path("flight_delay_map_poster.png")
    plt.savefig(OUT_PNG_POSTER, bbox_inches='tight', dpi=300)
    print(f"✅ Poster-friendly map saved to '{OUT_PNG_POSTER}'")
else:
    print("⚠️ Not enough data to create poster version with modified colors")
