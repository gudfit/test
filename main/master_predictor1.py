...
from sklearn.preprocessing import MinMaxScaler, OrdinalEncoder # Added MinMaxScaler
...
class MasterPredictor:
    ...
    otherwise falls back to the regressor model. Returns human-readable status.
    """
    def __init__(self):
        # ... (Initialization code remains the same - loads models/stats) ...
    ...
    # ... (_load_* methods remain the same) ...
    def _load_chain_classifier_stats(self):
        ...
        try:
            ...
            if scaler_params and 'min' in scaler_params and 'scale' in scaler_params:
                self.chain_scaler = MinMaxScaler() # Now imported
    ...
    def _load_chain_classifier_model(self):
        ...
        try:
            ...
            print("Chain Classifier model loaded successfully.")
             ...
             print("Architecture/hyperparameters mismatch.")
    ...
    # --- Helper function to map minutes to category ---
    def _get_category_from_minutes(self, minutes, thresholds):
        ...
        # Thresholds: [-inf, 15, 60, 120, 240, inf]
        # Category 0: delay <= 15
        # Category 1: 15 < delay <= 60
        # ...
        for i in range(len(thresholds) - 1):
            # Check if minutes falls between thresholds[i] (exclusive lower) and thresholds[i+1] (inclusive upper)
            if thresholds[i] < minutes <= thresholds[i+1]:
                return i
        return -1 # Should not happen with infinity bounds, but safety fallback
    ...
    def predict(self, flight_context_dicts):
        ...
        prediction_details = "" # Store extra info for message
        ...
        if use_classifier:
            ...
            try:
                ...
                if processed_chain_tensor is None:
                     ...
                     prediction_type = 'regressor_fallback' # Mark fallback
                     # Fall through to regressor logic below
                else:
                    with torch.no_grad():
                ...
                traceback.print_exc()
                prediction_type = 'regressor_fallback' # Mark fallback
                # Fall through to regressor logic

        # --- Fallback or Default: Use Regressor ---
        if not use_classifier or prediction_type == 'regressor_fallback':
             prediction_type = 'regressor' if prediction_type != 'regressor_fallback' else prediction_type
             ...
             try:
                ...
                if pd.isna(sched_dep_target):
                     return {'prediction_type': prediction_type, 'value': None, 'status': 'error', 'message': 'Target flight schedule time is invalid.'}
                ...
                predicted_delay_minutes = max(0.0, float(predicted_delay_minutes)) # Ensure float and non-negative
                ...
                # Convert minutes to category status string
                category_index = self._get_category_from_minutes(
                    predicted_delay_minutes,
                    self.config.CHAIN_DELAY_THRESHOLDS
                )
                ...
                return {
                    'prediction_type': prediction_type,
                    'value': status_string, # Return the string status
                    'status': 'success',
                ...
                traceback.print_exc()
                return {'prediction_type': prediction_type, 'value': None, 'status': 'error', 'message': f'Error during regressor prediction: {e}'}
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\nInitialization Failed: Could not find necessary model/stats file: {e}")
        sys.exit(1)
    except ImportError as e:
        print(f"\nInitialization Failed: Could not import necessary modules: {e}")
        sys.exit(1)
    except Exception as e:
